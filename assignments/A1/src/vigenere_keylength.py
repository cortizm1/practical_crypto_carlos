...
def ioc_matrix_frequency_computer(column_:np.ndarray):
    try:
        ...
        column_frequencies_ = []
        [column_frequencies_.append(list(column_).count(i)) for i in unique_] #using lists instead of the would be numpy equivalents (easier)

        # pseudo recursive frequency of frequencies thingy
        unique_ = list(set(column_frequencies_))
        column_frequencies_final = [(i, column_frequencies_.count(i)) for i in unique_]
        return column_frequencies_final, len(column_)
    except Exception as e:
        error_writer(e, description=f"Error while computing per column frequency tables.")

@function_logger_too
def p_squared(frequency_tuple_:tuple, blob_length:int):
    try:
        freq_, ntimes_ = frequency_tuple_
        return ntimes_*(freq_/blob_length)**2
    except Exception as e:
        error_writer(e, description=f"Error while computing p_squared.")

@function_logger_too
def ioc_matrix_ioc_computer(frequency_tuples, blob_length:int):
    try:
        sum_p_squared = 0.00
        frequency_tuples = frequency_tuples[0]
        sum_p_squared = sum([p_squared(tuple_, blob_length) for tuple_ in frequency_tuples])
        return round(sum_p_squared, 4)
    except Exception as e:
...
def ioc_matrix_computer(ciphertext_matrix:np.ndarray, blob_length:int):
    try:
        ...
        ioc_calcs_ = [ioc_matrix_ioc_computer(list_tuple_, column_freqs_) for *list_tuple_, column_freqs_ in matrix_frequencies]
...
def ioc_search(extracted_text_blob:list, blob_length:int): #needs to be numpy array/matrix
    try:
        ciphertext_matrices = []
        [ciphertext_matrices.append([i, ciphertext_digitizer_and_matrixator(extracted_text_blob, i)]) for i in range(2,33)]
...
def main(argv=None):
    ...
    ap = argparse.ArgumentParser(prog="vigenere_keylength.py", description=description,
                                 formatter_class=argparse.RawDescriptionHelpFormatter)

    logging.basicConfig(filename='vigenere_keylength.log', level=logging.INFO)
